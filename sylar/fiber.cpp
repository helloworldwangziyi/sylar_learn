...
Fiber::~Fiber()
{
    ...
    else
    {
        ...
        SYLAR_ASSERT(m_state == EXEC);    // 主协程一定是执行状态

        Fiber* cur = t_fiber;
        if(cur == this)
        {
            SetThis(nullptr);  // 主协程析构，将当前协程置空
        }
    }
    SYLAR_LOG_DEBUG(g_logger) << "Fiber::~Fiber id = " << m_id
        << " total = " << s_fiber_count;
...
void Fiber::swapOut()
{
    SetThis(t_threadFiber.get()); // 将当前协程置为主协程
...
{
    Fiber::ptr cur = GetThis();
    SYLAR_ASSERT(cur); // 保证当前协程一定存在
    try{ // 执行协程函数
        cur->m_cb();
        cur->m_cb = nullptr; // 执行完毕，将cb置空
        cur->m_state = TERM;
    }
